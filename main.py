...
# Objetivos: Entrada de dados, botões e verificação básica de login
    ...
    def frame_login(self):
        ...
        frame = ctk.CTkFrame(self, width=300, height=300, fg_color="blue")  # Cria a tela do login e ajusta o tamanho da caixa   # Cria a tela de login
        ...
        self.tag_user = ctk.CTkLabel(frame, text="Usuário:")
        self.tag_user.pack(anchor="w", padx=86)
    
        # Entrada de senha
        self.entry_pass = ctk.CTkEntry(frame, placeholder_text="Senha", show="*")
        self.entry_pass.pack(pady=(0,10))

        self.tag_pass = ctk.CTkLabel(frame, text="Senha:")
        self.tag_pass.pack(anchor="w", padx=86)
...
# Objetivos: Criação de Layout com Menu Lateral e Área de Conteúdo
    ...
    def frame_main(self):
        ...
        btn_emprestimos = ctk.CTkButton(menu_frame, text="Gerenciar Empréstimos", command=self.frame_emprestimo)
...
# Inserir, editar e alternar status.
    ...
    def atualizar_botao_status(self, tree=None, btn_status=None):
        ...
        if tree is None or btn_status is None:
            return # Se não receber tabela ou botão, não faz nada
    ...
    def deletar_cliente(self):
        ...
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM Emprestimo WHERE id_cliente=%s", (cliente_id, ))
        count = cursor.fetchone() [0]

        if count > 0:
            messagebox.showwarning("Erro", "O Cliente selecionado está vinculado a tabela Empréstimo ",)
            conn.close()
            return
        else:
    ...
    def salvar_livro_janela(self, titulo_livro, autor_livro, ano, genero, status, win, id_livro=None):
        ...
        if not id_livro:
            ...
            msg = f"Livro atualizado com sucesso!"
    ...
    def deletar_livro(self):
        ...
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM Emprestimo WHERE id_livro=%s", (livro_id, ))
        count = cursor.fetchone() [0]

        if count > 0:
            messagebox.showwarning("Erro", "O Livro selecionado está vinculado a tabela Emprestimo ")
            conn.close()
            return
        else:
    ...
    def inserir_emprestimo(self):
        ...
        win.attributes("-topmost", True) # mantém na frente temporariamente
    ...
    def salvar_emprestimo(self, cliente_str, livro_str, win):
        ...
        # Gera ID do empréstimo no automáticamente aleatório no formato E1234
    ...
    def editar_emprestimo(self):
        ...
        if not item:
            messagebox.showwarning("Aviso", f"Selecione um empréstimo para editar.")
            return
    ...
    def salvar_edicao_emp(self, id_emp, nova_data, win):
        try:
            ...
            messagebox.showerror("Error", "Data inválida (use AAAA-MM-DD)")
            return
        conn = conectar()
        cursor = conn.cursor()
